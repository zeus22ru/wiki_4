...
SPECS: list[SettingSpec] = [
    ...
    SettingSpec(
        ...
        env="CHAT_ATTACHMENTS_ENABLED",
        group=_GROUP_API,
        type="bool",
        description="Разрешить вложения к вопросу в чате (изображения и текстовые файлы).",
        allowed="true/false",
    ),
    SettingSpec(
        key="CHAT_ATTACHMENTS_DIR",
        env="CHAT_ATTACHMENTS_DIR",
        group=_GROUP_STORAGE,
        type="str",
        description="Папка хранения вложений чата.",
        allowed="путь к директории",
    ),
    SettingSpec(
        key="CHAT_ATTACHMENT_MAX_COUNT",
        env="CHAT_ATTACHMENT_MAX_COUNT",
        group=_GROUP_API,
        type="int",
        description="Максимум вложений на один вопрос.",
        allowed="целое число от 1 до 10",
        ui={"kind": "slider", "min": 1, "max": 10, "step": 1},
    ),
    SettingSpec(
        key="CHAT_ATTACHMENT_MAX_BYTES",
        env="CHAT_ATTACHMENT_MAX_BYTES",
        group=_GROUP_API,
        type="int",
        description="Максимальный размер одного вложения (байты).",
        allowed="целое число ≥ 1 (например 5242880 = 5MB)",
    ),
    SettingSpec(
        key="CHAT_ATTACHMENT_TEXT_MAX_CHARS",
        env="CHAT_ATTACHMENT_TEXT_MAX_CHARS",
        group=_GROUP_API,
        type="int",
    ...
    SettingSpec(
        ...
        env="CHAT_ATTACHMENT_ALLOWED_EXTENSIONS",
        group=_GROUP_API,
        type="list",
        description="Разрешённые расширения вложений чата (через запятую).",
        allowed='png,jpg,jpeg,webp,gif,txt,log,md,json,xml,csv,yaml,yml,ini,env',
    ),
    # Bitrix24
    SettingSpec(
